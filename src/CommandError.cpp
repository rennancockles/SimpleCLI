...
CommandError::CommandError(cmd_error *errorPointer, bool persistent)
    : errorPointer(errorPointer), persistent(persistent) {
...
bool CommandError::operator==(const CommandError &e) const {
    return errorPointer == e.errorPointer ||
           (errorPointer && e.errorPointer && errorPointer->mode == e.errorPointer->mode &&
            errorPointer->command == e.errorPointer->command &&
            errorPointer->argument == e.errorPointer->argument && errorPointer->data == e.errorPointer->data);
}

bool CommandError::operator!=(const CommandError &e) const { return !(*this == e); }
...
CommandError::operator bool() const { return errorPointer && errorPointer->mode != CMD_PARSE_SUCCESS; }

bool CommandError::hasCommand() const { return errorPointer && errorPointer->command; }

bool CommandError::hasArgument() const { return errorPointer && errorPointer->argument; }

bool CommandError::hasData() const { return errorPointer && errorPointer->data; }
...
CommandErrorType CommandError::getType() const {
    if (errorPointer) {
        switch (errorPointer->mode) {
            ...
            case CMD_CALLBACK_FAILURE: return CommandErrorType::CALLBACK_FAILURE;
        }
    }
    return CommandErrorType::PARSE_SUCCESSFUL;
}

bool CommandError::hasCmd() const { return hasCommand(); }

bool CommandError::hasArg() const { return hasArgument(); }

Command CommandError::getCommand() const { return Command(errorPointer ? errorPointer->command : NULL); }

Argument CommandError::getArgument() const { return Argument(errorPointer ? errorPointer->argument : NULL); }
...
String CommandError::getMessage() const {
    if (errorPointer) {
        switch (errorPointer->mode) {
            ...
            case CMD_CALLBACK_FAILURE: return String(F("Failed running callback"));
        }
    }
    return String();
}

Command CommandError::getCmd() const { return getCommand(); }

Argument CommandError::getArg() const { return getArgument(); }

String CommandError::getMsg() const { return getMessage(); }
...
cmd_error *CommandError::getPtr() { return errorPointer; }